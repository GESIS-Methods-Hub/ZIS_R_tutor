...
author:
  ...
  - name: Zoe Greer
    affiliations: "Victoria University of Wellington; GESIS – Leibniz Institute for the Social Sciences"
...
What should you pay attention to while trying to validate a scale? What analyses do you need to conduct to get your instrument published in a journal or archived in a repository?
This tutorial presents the basic analyses that you usually need to do to make your scale publicly available, with a focus on computation. Specifically, you will learn:

- how to compute descriptive statistics for items and their aggregates (e.g., mean scores),
- how to explore the dimensionality of the data,
- how to fit confirmatory factor analytic models and choose the optimal solution,
- how to calculate different reliability coefficients,
- and which commonly used procedures you should approach with caution.

As an example, we use a scale with continuous indicators, so not all parts of the analysis are appropriate for instruments with categorical indicators. We highlight this whenever relevant. The treatment of categorical indicators is covered in a separate tutorial.
...
Now that you have completed the tutorial, you should be able to conduct the basic analyses needed to evaluate the psychometric qualities of scales with continuous indicators. You have learned which descriptive statistics are typically reported, how to explore the dimensionality of the data using parallel analysis, and how to run a confirmatory factor analytic model and understand the key elements of its output. Additionally, you should be able to calculate different reliability metrics, and know which one to use when the items are non-homogeneous. Finally, you should now understand how to investigate criterion validity by ensuring that the scales capture the intended construct by evaluating whether the direction and magnitude of correlations with external variables aligns with expectations.
